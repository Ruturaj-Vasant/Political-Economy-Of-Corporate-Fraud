...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser("Enrich metadata JSON with WRDS identifiers")
    ...
    updated, changes_df = enrich_records(data, batch_size=args.batch_size, limit=args.limit, dry_run=args.dry_run)

    if args.dry_run:
        print(f"Dry run complete. {len(changes_df)} changes would be applied.")
    else:
        if args.in_place:
            ...
            out_path = Path(args.output) if args.output else inp.with_name(inp.stem + ".enriched.json")
