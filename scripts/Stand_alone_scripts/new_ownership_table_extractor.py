...
def _collect_head_includes(tree) -> str:
    """Return stylesheet/link tags to preserve original table styling."""
    nodes = tree.xpath(
...
def extract_beneficial_ownership_tables(
    html_bytes: bytes, top_k: int = 3, base_href: Optional[str] = None
    ...
    for idx, tbl in enumerate(tables):
        ...
        stub = _html_stub(
            tbl_html,
            f"Beneficial Ownership Extract ({kind}) - table[{idx}] score={score:.2f}",
            base_href=base_href,
...
def main() -> int:
    ...
    scored = extract_beneficial_ownership_tables(
        html_bytes, top_k=max(args.top_k, 2), base_href=in_path.parent.as_uri()
    )
    if not scored:
